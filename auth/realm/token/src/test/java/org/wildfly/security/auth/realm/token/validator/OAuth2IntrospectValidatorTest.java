...
public class OAuth2IntrospectValidatorTest {
    ...
    public void testBuildParametersEncodesInjectionCharacters() throws Exception {
        OAuth2IntrospectValidator validator = OAuth2IntrospectValidator.builder()
                .clientId("test-client")
                .clientSecret("test-secret")
                .tokenIntrospectionUrl(new java.net.URL("http://localhost/introspect"))
                .build();

        Method buildParameters = OAuth2IntrospectValidator.class.getDeclaredMethod("buildParameters", Map.class);
        buildParameters.setAccessible(true);

        Map<String, String> parameters = new HashMap<>();
        parameters.put("token", "legitimate_token&injected=evil");
        parameters.put("token_type_hint", "access_token");

        byte[] result = (byte[]) buildParameters.invoke(validator, parameters);
