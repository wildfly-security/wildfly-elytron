...
public class BasicIntegrityTest extends AbstractTest {
    ...
    public void testAuthInt() throws Exception {
        ...
        exchange = evaluateByServer(exchange);
        assertEquals("603f06092a864886f71201020202010400ffffffffd81ba0780a9c56021fe68eaae0773020093ba16f79a534ae5ddb7576e687fe8bc940ea4a0200fb4104040404", ByteIterator.ofBytes(exchange).hexEncode().drainToString());
        assertFalse(server.isComplete());
        assertFalse(client.isComplete());

        exchange = evaluateByClient(exchange);
        assertEquals("603f06092a864886f71201020202010400ffffffff935e0a7e1a1642ed9dec1f922ddf3d9534b4e203144b25213bde59019cd068a60d1fd2ad0200ef3204040404", ByteIterator.ofBytes(exchange).hexEncode().drainToString());
        assertFalse(server.isComplete());
        ...
        assertEquals("604706092a864886f71201020202010400ffffffffc91dc34545ff367848c0e08bb9964a6078ab314cc07ab097588ee37bdd0034311f4ac2c400123456789abcdeff07070707070707", ByteIterator.ofBytes(wrappedMessage).hexEncode().drainToString());
        ...
        assertEquals("604706092a864886f71201020202010400ffffffff0c3fd2064ccc0888c203a8fa13de89b8edd1fd9e782fb5ce39dd9cc618a2ea74e2888d3fffedcba9876543210007070707070707", ByteIterator.ofBytes(wrappedMessage).hexEncode().drainToString());
