...
import org.wildfly.security.vault._private.ClassModuleSpec;
 ...
 */
public final class VaultCallbackHandler implements CallbackHandler {

    /**
     * Default constructor.
     */
    public VaultCallbackHandler() {}
    ...
    public void handle(Callback[] callbacks) throws IOException, UnsupportedCallbackException {
        ...
        if (sm != null) {
            sm.checkPermission(VaultPermission.LOAD_EXTERNAL_STORAGE_PASSWORD);
        }
        ...
        for (Callback callback: callbacks) {
            if (callback instanceof ExtCallback) {
                ...
                try {
                    if (sm != null) {
                        extCallback.setPassword(ExecRuntimeActions.PRIVILEGED.execCmd(extCallback.getArgLine()));
                    } else {
                        extCallback.setPassword(ExecRuntimeActions.NON_PRIVILEGED.execCmd(extCallback.getArgLine()));
                    }
                } catch (Exception e) {
                ...
                try {
                    if (sm != null) {
                        cmdCallback.setPassword(CmdRuntimeActions.PRIVILEGED.execCmd(cmdCallback.getArgs()));
                    } else {
                        cmdCallback.setPassword(CmdRuntimeActions.NON_PRIVILEGED.execCmd(cmdCallback.getArgs()));
                    }
                } catch (Exception e) {
                    throw new IOException(e);
                }
                ...
                try {
                    classCallback.setPassword(invokePasswordClass(
    ...
    private static char[] invokePasswordClass(final ClassModuleSpec passwordClassSpec, final Map<String, ?> passwordClassParameters, final Object[] passwordClassArguments) throws VaultException {
