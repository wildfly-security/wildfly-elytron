...
final class DigestMD5Client extends DigestMD5Base implements SaslClient {
    ...
    private static final Logger log = Logger.getLogger("org.wildfly.sasl.digest.client");
    ...
    DigestMD5Client(String authzid, String protocol, String serverName,
        ...
        if (props != null) {
            ...
            log.tracef("DIGEST60:Explicitly specified cipher: %s", specifiedCipher);
            ...
            } else if (props.containsKey(LEGACY_PRE_DIGESTED_PROPERTY)) {
                preDigestedPasswords = Boolean.parseBoolean(String.valueOf(props.get(LEGACY_PRE_DIGESTED_PROPERTY)));
                log.tracef("Server using pre-digested hashes (%B)", preDigestedPasswords);
            }
        }
   }
    ...
    */
    private void processChallenge(byte[][] challengeVal, List<byte[]> realmChoices)
        ...
        try {
            ...
            if (realmTokens == null) {
                ...
                cbh.handle(new Callback[] {tcb, ncb, credentialCallback});
                ...
                RealmChoiceCallback ccb = new RealmChoiceCallback(
                    ...
                    realmTokens,
                    0, false);
                cbh.handle(new Callback[] {ccb, ncb, credentialCallback});
        ...
        if (username == null || (passwd == null && preDigested == null)) {
            throw new SaslException(
    ...
    private void checkQopSupport(byte[] qopInChallenge, byte[] ciphersInChallenge)
        ...
        log.tracef("DIGEST61:Raw send size: %d", rawSendSize);
    }
    ...
    private void checkStrengthSupport(byte[] ciphersInChallenge)
        ...
        for (int i = 0; i < tokenCount; i++) {
            ...
            for (int j = 0; j < CIPHER_TOKENS.length; j++) {
                if (token.equals(CIPHER_TOKENS[j])) {
                    ...
                    log.tracef("DIGEST62:Server supports %s", token);
                }
            }
        }
        ...
        log.tracef("DIGEST63:Cipher suite: %s", negotiatedCipher);
    }
        ...
        try {
            ...
            if (preDigestedPasswords) {
                digestResp.write(generateResponseValue("AUTHENTICATE",
                    digestUri, negotiatedQop,
                    preDigested, nonce, cnonce,
    ...
    private void clearPassword() {
        ...
        if (preDigested != null) {
            for (int i = 0; i < preDigested.length; i++) {
                preDigested[i] = 0;
            }
            preDigested = null;
        }
