...
public class DERDecoderTest {
    ...
    // --- Tests for ELY-3060: inflated length field must be rejected, not allocated ---

    private static final String INFLATED_LENGTH_MESSAGE = "ELY28000: Declared length exceeds available bytes in DER encoded data";

    /**
     * Crafts a DER payload where the length field declares 100 bytes of content
     * but only 1 byte is actually present. The decoder must reject this before
     * attempting allocation.
     *
     * Layout: tag (1 byte) + length 100 (1 byte) + 1 byte dummy content = 3 bytes total.
     */
    private static byte[] inflatedPayload(int tag) {
        return new byte[] {
                (byte) tag,
                100,               // declares 100 bytes of content
                0x00               // only 1 byte actually present
        };
    }

    private static void assertInflatedLengthRejected(Runnable decode) {
        try {
            decode.run();
            fail("Expected ASN1Exception for inflated length");
        } catch (ASN1Exception e) {
            assertEquals(INFLATED_LENGTH_MESSAGE, e.getMessage());
        }
    }

    @Test
    public void testDecodeOctetStringInflatedLength() {
        assertInflatedLengthRejected(() -> new DERDecoder(inflatedPayload(0x04)).decodeOctetString());
    }

    @Test
    public void testDecodeOctetStringAsStringInflatedLength() {
        assertInflatedLengthRejected(() -> new DERDecoder(inflatedPayload(0x04)).decodeOctetStringAsString());
    }

    @Test
    public void testDecodeBitStringInflatedLength() {
        assertInflatedLengthRejected(() -> new DERDecoder(inflatedPayload(0x03)).decodeBitString());
    }

    @Test
    public void testDecodeBitStringAsStringInflatedLength() {
        assertInflatedLengthRejected(() -> new DERDecoder(inflatedPayload(0x03)).decodeBitStringAsString());
    }

    @Test
    public void testDecodeIA5StringInflatedLength() {
        assertInflatedLengthRejected(() -> new DERDecoder(inflatedPayload(0x16)).decodeIA5String());
    }

    @Test
    public void testDecodePrintableStringInflatedLength() {
        assertInflatedLengthRejected(() -> new DERDecoder(inflatedPayload(0x13)).decodePrintableString());
    }

    @Test
    public void testDecodeUtf8StringInflatedLength() {
        assertInflatedLengthRejected(() -> new DERDecoder(inflatedPayload(0x0C)).decodeUtf8String());
    }

    @Test
    public void testDecodeBMPStringInflatedLength() {
        assertInflatedLengthRejected(() -> new DERDecoder(inflatedPayload(0x1E)).decodeBMPString());
    }

    @Test
    public void testDecodeUniversalStringInflatedLength() {
        assertInflatedLengthRejected(() -> new DERDecoder(inflatedPayload(0x1C)).decodeUniversalString());
    }

    @Test
    public void testDrainElementValueInflatedLength() {
        assertInflatedLengthRejected(() -> new DERDecoder(inflatedPayload(0x04)).drainElementValue());
    }

    @Test
    public void testDrainElementInflatedLength() {
        assertInflatedLengthRejected(() -> new DERDecoder(inflatedPayload(0x04)).drainElement());
    }

    @Test
    public void testStartSequenceInflatedLength() {
        assertInflatedLengthRejected(() -> new DERDecoder(inflatedPayload(0x30)).startSequence());
    }
